...
from peptron.data import data_pipeline, feature_pipeline
from peptron.utils.tensor_utils import tensor_tree_map, dict_multimap

from openfold.np import residue_constants
from openfold.data.data_transforms import make_atom14_masks
import peptron.data.protein as protein
...
class CSVDataset:
    ...
    def __getitem__(self, idx):
        ...
        seqlen = len(row.seqres)
        batch = {
            'name': row.name,
            'seqres': row.seqres,
            'aatype': seq_to_tensor(row.seqres),
            'residue_index': torch.arange(seqlen),
            'pseudo_beta_mask': torch.ones(seqlen),
            'seq_mask': torch.ones(seqlen),
            'seq_length': torch.tensor([seqlen]*seqlen, dtype=torch.int32),
        }
