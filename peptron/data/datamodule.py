...
from peptron.data.data import OpenFoldSingleDataset, OpenFoldBatchCollator, OpenFoldDataset
...
from peptron.utils.util import str_to_ascii


Mode = Literal["train", "validation", "test", "predict"]

DATASET_TYPES = Union[OpenFoldSingleDataset, OpenFoldDataset, None]


def structure_data_step(dataloader_iter) -> Dict[str, Tensor]:
    ...
    """Custom data step that preserves structure data."""
    batch = next(dataloader_iter)

    # The OpenFoldBatchCollator already stacks tensors along dim=0
    # and returns a dictionary directly, so we don't need the tuple unpacking
    _batch = batch[0]

    # Add structure keys to required keys
    required_keys = set()
    # ESM model inputs
    required_keys.add("input_ids")
    
    if parallel_state.is_pipeline_first_stage():
        required_keys.add("name")  # Add name field for identification
        required_keys.add("resolution")
    ...
    for key, val in _batch.items():
        if key in required_keys:
            if key == "name":
                val = str_to_ascii(val)
            _parallel_batch[key] = val.cuda(non_blocking=True)
        else:
            _parallel_batch[key] = None

    # slice batch along sequence dimension for context parallelism
    output = get_batch_on_this_context_parallel_rank(_parallel_batch)
    return output


class ESMFoldDataModule(MegatronDataModule):
    """A PyTorch Lightning DataModule for training ESMFold models.

    This DataModule is designed to handle the data preparation and loading for training ESMFold models.
    ...
    def _create_epoch_based_dataset(
        self,
        dataset: OpenFoldSingleDataset | OpenFoldDataset,
        total_samples: int,
    ):
    ...
    def train_dataloader(self) -> TRAIN_DATALOADERS:
        ...
        assert self._train_ds is not None, "train_dataset is not provided to ESMFoldDataModule"
        return self._create_dataloader(self._train_ds, mode="train")

    def val_dataloader(self) -> EVAL_DATALOADERS:
        """Returns the dataloader for validation data."""
        assert self._valid_ds is not None, "valid_dataset is not provided to ESMFoldDataModule"
        return self._create_dataloader(self._valid_ds, mode="validation")

    def predict_dataloader(self) -> EVAL_DATALOADERS:
        """Returns the dataloader for prediction data."""
        assert self.predict_dataset is not None, "predict_dataset is not provided to ESMFoldDataModule"
        return self._create_dataloader(self.predict_dataset, mode="predict")

    def test_dataloader(self) -> EVAL_DATALOADERS:
        """Raises a not implemented error."""
        raise NotImplementedError("No test dataset provided for ESMFold")