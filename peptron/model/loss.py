...
class AlphaFoldLoss(nn.Module):
    ...
    def forward(self, out, batch, _return_breakdown=False):
        ...
        del loss_fns["experimentally_resolved"], loss_fns["masked_msa"]
...
class ESMFoldLossReduction(BERTMLMLossWithReduction):
    """A class for calculating the Structure losses of folding output.
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.structure_loss = AlphaFoldLoss(LossConfig())

    def forward(
            self, batch, forward_out
    ):
        ...
        if cp_size == 1:
            loss, _ = self.structure_loss(forward_out, batch, _return_breakdown=True)
    ...
    def reduce(self, losses_reduced_per_micro_batch) -> Tensor:
...
class ConfigMappingMixin(abc.Mapping):
    def __len__(self):
...
@dataclass
class DistogramConfig(iom.IOMixinWithGettersSetters, ConfigMappingMixin):
...
@dataclass
class ExperimentallyResolvedConfig(iom.IOMixinWithGettersSetters, ConfigMappingMixin):
    eps: float = 1e-8
    min_resolution: float = 0.1
    max_resolution: float = 3.0
    weight: float = 0.0
...
@dataclass
class FapeBackboneConfig(iom.IOMixinWithGettersSetters, ConfigMappingMixin):
    clamp_distance: float = 10.0
    loss_unit_distance: float = 10.0
    use_clamped_fape: float = 1.0
    weight: float = 0.5


@dataclass
class FapeSidechainConfig(iom.IOMixinWithGettersSetters, ConfigMappingMixin):
    clamp_distance: float = 10.0
    length_scale: float = 10.0
    weight: float = 0.5
...
@dataclass
class FapeConfig(iom.IOMixinWithGettersSetters, ConfigMappingMixin):
...
@dataclass
class PlddtLossConfig(iom.IOMixinWithGettersSetters, ConfigMappingMixin):
    min_resolution: float = 0.1
    max_resolution: float = 3.0
    cutoff: float = 15.0
    no_bins: int = 50
...
@dataclass
class MaskedMSAConfig(iom.IOMixinWithGettersSetters, ConfigMappingMixin):
    eps: float = 1e-8
    weight: float = 2.0


@dataclass
class SupervisedChiConfig(iom.IOMixinWithGettersSetters, ConfigMappingMixin):
...
@dataclass
class ViolationConfig(iom.IOMixinWithGettersSetters, ConfigMappingMixin):
...
@dataclass
class TMConfig(iom.IOMixinWithGettersSetters, ConfigMappingMixin):
    max_bin: int = 31
    no_bins: int = 64
    min_resolution: float = 0.1
    max_resolution: float = 3.0
...
@dataclass
class LossConfig(iom.IOMixinWithGettersSetters, ConfigMappingMixin):
    distogram: DistogramConfig = field(default_factory=DistogramConfig)
    experimentally_resolved: ExperimentallyResolvedConfig = field(default_factory=ExperimentallyResolvedConfig)
...
def get_loss_reduction_class(loss_config: LossConfig) -> Type[ESMFoldLossReduction]:
    class ESMFoldLossReductionConfigured(ESMFoldLossReduction):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.structure_loss = AlphaFoldLoss(loss_config)

    return ESMFoldLossReductionConfigured


def loss_config_from_configdict(config: ConfigDict) -> LossConfig:
    return LossConfig(
        distogram=DistogramConfig(**config.distogram),
        experimentally_resolved=ExperimentallyResolvedConfig(**config.experimentally_resolved),
        plddt_loss=PlddtLossConfig(**config.plddt_loss),
        masked_msa=MaskedMSAConfig(**config.masked_msa),
        supervised_chi=SupervisedChiConfig(**config.supervised_chi),
        violation=ViolationConfig(**config.violation),
        tm=TMConfig(**config.tm),
        fape=FapeConfig(
            backbone=FapeBackboneConfig(**config.fape.backbone),
            sidechain=FapeSidechainConfig(**config.fape.sidechain),
            eps=config.fape.eps,
            weight=config.fape.weight,
        ))
