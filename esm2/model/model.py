...
from esm2.data.tokenizer import BioNeMoESMTokenizer
from esm2.model.attention import ESM2DotProductAttention, ESM2TEDotProductAttention
from esm2.model.embedding import ESM2Embedding
from bionemo.llm.api import MegatronLossType
from bionemo.llm.model.biobert.model import BioBertConfig, MegatronBioBertModel, PositionEmbeddingKinds, BioBertOutput, \
...
class ESM2Model(MegatronBioBertModel):
    ...
    def forward(
        ...
        # Run custom encoder to output all_hidden_states
        hidden_states, all_hidden_states = self.encoder_forward(
        ...
        if self.include_hiddens:
            ...
            output["all_hidden_states"] = torch.stack(all_hidden_states) # TODO: check dim when testing
    ...
    def encoder_forward(
        ...
        if not self.encoder.pre_process:
        ...
        if inference_params and not self.encoder.training:
        ...
        if self.encoder.config.sequence_parallel:
            rng_context = tensor_parallel.get_cuda_rng_tracker().fork()
        else:
            rng_context = nullcontext()

        if self.encoder.config.fp8:
            import transformer_engine  # To keep out TE dependency when not training in fp8

            if self.encoder.config.fp8 == "e4m3":
                fp8_format = transformer_engine.common.recipe.Format.E4M3
            elif self.encoder.config.fp8 == "hybrid":
            ...
            fp8_recipe = TEDelayedScaling(
                ...
                override_linear_precision=(False, False, not self.encoder.config.fp8_wgrad),
            )
            fp8_group = None
            if parallel_state.model_parallel_is_initialized():
                fp8_group = parallel_state.get_amax_reduction_group(
                    with_context_parallel=True, tp_only_amax_red=self.encoder.tp_only_amax_red
                )
        ...
        with rng_context, fp8_context:
            # Forward pass.
            if self.encoder.config.recompute_granularity == 'full' and self.encoder.training:
                hidden_states = self.encoder._checkpointed_forward(
                ...
                for l_no, layer in enumerate(self.encoder.layers):
                    with self.encoder.offload_context:
                        ...
                        if (len(self.encoder.cuda_graphs) == 0) or (not self.encoder.training):
                            ...
                            cg_index = self.encoder.current_microbatch % len(self.encoder.cuda_graphs[l_no])
                            assert not any(
                                [inference_params, packed_seq_params]
                            ), "CUDA graph accepts only Tensor inputs."
                            optional_inputs = self.encoder.get_cuda_graph_optional_args(
                                attention_mask,
                                context,
                                context_mask,
                                rotary_pos_emb,
                            ...
                            hidden_states = self.encoder.cuda_graphs[l_no][cg_index](
                                hidden_states, **optional_inputs
                            )

                    if (
                        torch.is_grad_enabled()
                        and self.encoder.config.cpu_offloading
                        and self.encoder.group_prefetch_offload_commit_async is not None
                    ):
                        hidden_states = self.encoder.group_prefetch_offload_commit_async(hidden_states)

                    if l_no < len(self.encoder.layers) - 1:
                        # TENorm produces a "viewed" tensor. This will result in schedule.py's
                        # deallocate_output_tensor() throwing an error, so a viewless tensor is
                        # created to prevent this.
                        hidden_states_ =  make_viewless_tensor(inp=hidden_states, requires_grad=True, keep_graph=True).permute(1, 0, 2)
                        all_hidden_states.append(hidden_states_)

        # Final layer norm.
        if self.encoder.final_layernorm is not None:
            ...
            hidden_states_ = hidden_states.permute(1, 0, 2)
            all_hidden_states.append(hidden_states_)
